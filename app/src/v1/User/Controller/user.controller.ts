...
import {
	ISerializeResponse,
	serializerService,
} from 'src/shared/Services/serializer.service';
...
export class UsersController {
	...
	async getOwnProfile(
		...
		const profile = await this.userService.getOwnProfile(bearer);
	...
	async updateOwnProfile(
		...
		const profile = await this.userService.updateOrCreateOwnProfile(
			bearer,
			dto,
		);
		return serializerService.serializeResponse(
	...
	async downloadProfileImage(
		...
		if (profile.image_size <= 0)
			return { status: 'failed', message: 'No image found' };
	...
	async getNoteList(
		...
		return serializerService.paginateResponse(
			'note_list',
			data,
			query,
			itemCount,
	...
	async addNote(
		...
		const note = await this.userService.addNote(bearer, dto);
	...
	@Delete('/notes/:id')
	async removeNote(@Param() param: SingleIDDto): Promise<StatusOk> {
